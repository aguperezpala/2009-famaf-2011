...
int Spawn(const char *program, const char *command, struct Kernel_Thread **pThread)
{
     ...
     *
     * EL COMENTARIO QUE SIGUE CONTRADICE LA ESPECIFICACION:
    ...
    if (rc != 0) {
	/*Error while parsing executable*/
	return ENOEXEC;
    }

    rc = Load_User_Program(buf, filelen, &exeformat, command, &pUsercontext);
    if (rc != 0) {
	/*Error while building user context*/
	return ENOMEM;
    }
