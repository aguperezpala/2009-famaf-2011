    ...
    struct Kernel_Thread* kthread, struct User_Context* userContext)
{
    ...
    Push(kthread, userContext->dsSelector); /*ss*/
    Push(kthread, userContext->stackPointerAddr);

    Push(kthread, 512L); /*EFLAGS with IF flag set*/

    Push(kthread, userContext->csSelector); /*cs*/
    Push(kthread, userContext->entryAddr); /*program counter*/

    Push(kthread, 0L); /*error code*/
    Push(kthread, 0L); /*interrupt number*/
...
struct Kernel_Thread* Lookup_Thread(int pid)
{
    ...
    while (result != 0) {
	if (result->pid == pid) {
	    /*if (g_currentThread != result->owner)	//comente esto para que me devuelva el pid aunque no sea el owner el que lo pide
		result = 0;*/
	    break;
	}
